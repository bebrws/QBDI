...
static void catch_sigint(int signum)
{
    ...
    list_for_each_entry(&modules, cur_module, mlist) {
      ...
      fprintf(fp, "%*d, 0x%llx, 0x%llx, 0x0000000000000000, 0x00000000, 0x00000000, %s\n", column_wdith, i, cur_module->start, cur_module->end, cur_module->path);
      i += 1;
    }
...
static VMAction onInstruction(VMInstanceRef vm, GPRState *gprState, FPRState *fprState, void *data) {
    ...
    char addr_key[32];
    snprintf(addr_key, 32,"%llx",instAnalysis->address);
    ...
    if (!val) {
      ...
      list_for_each_entry(&modules, cur_module, mlist) {
        which_module += 1;
        if (instAnalysis->address >= cur_module->start && instAnalysis->address <= cur_module->end) {
          break;
        }
      }

      bb_wrapper *bbw = malloc(sizeof(bb_wrapper));
      bbw->bb.id = which_module;
      bbw->bb.start = instAnalysis->address - cur_module->start;
      bbw->bb.size = cur_module->end - cur_module->start;
